...
def euclidean_distance(loc1: Position, loc2: Position) -> float:
    ...
    try:
        x1, y1 = loc1
        x2, y2 = loc2
        eucledian_dist = math.sqrt(((x2 - x1)**2) + ((y2 - y1)**2))
...
def mutate_sentences(sentence: str) -> List[str]:
    ...
    try: 
        ...
        adj_words = collections.defaultdict(set)
        for i in range(len(words)-1):
            adj_words[words[i]].add(words[i+1])
        
        def sentence_builder(sentence):
            if len(sentence) == len(words):
                similar_sentence.append(' '.join(sentence))
            else:
                for adj_word in adj_words[sentence[-1]]:
                    sentence_builder(sentence+[adj_word])
...
def find_nonsingleton_words(text: str) -> Set[str]:
    ...
    try:
        ...
        for word in words:
            count[word] += 1
        non_singleton_set = set([w for w, c in count.items() if c>1])
        return non_singleton_set
    except:
        raise Exception("Not implemented yet")
    # END_YOUR_CODE
